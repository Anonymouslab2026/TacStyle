...
class TactileSensor:
    ...
    def turn_off_t_s_collisions(self):
        """
        Turn off collisions for selected tactile sensor links.
        """

        if self.t_s_name == "digit":
            # Keep DIGIT body collision enabled, disable only the tip/pad collision
            self._pb.setCollisionFilterGroupMask(self.robot_id, self.tactile_link_ids["tip"], 0, 0)
            return

        # Default behavior for other tactile sensors
        ...
        if self.t_s_name == "tactip" and self.t_s_core == "no_core":
            self._pb.setCollisionFilterGroupMask(self.robot_id, self.tactile_link_ids["tip"], 0, 0)

    def load_reference_images(self):
        # get saved reference images
        border_images_path = os.path.join(self.assets_dir, "urdf", self.t_s_name, "reference_images").replace("\\", "/")
    ...
    def save_reference_images(self):
        ...
        # TODO: check if saved_dile_dir exists
    ...
    def update_cam_frame(self):
        ...
        if self.t_s_name == 'tactip':
            ...
            if self.t_s_type in ["standard"]:
                cam_pos = (-0.00095, .0139, 0.)               # NOTE: cam_pos and cam_rpy had to be changed for digit with fairino
                cam_rpy = (np.pi, -np.pi/2, -0.020)
            elif self.t_s_type in ["right_angle", "forward"]:
    ...
    def t_s_camera(self):
        ...
        max_penetration = 0.1           # NOTE: increased from 0.05 to delay saturation?
